  ...
  public void etroll()
     {
        ...
        String NUMS = null,sql = null;
        int SUSTRING = 0;
           try{
          if(journals.journal1.devise.getText().equals("USD")){
               sql="SELECT * FROM  transaction_number WHERE PROJET='"+journals.journal1.buss.getText()+"' and NAME='BSC-USD'";
               }else if(journals.journal1.devise.getText().equals("CDF")){
               sql="SELECT * FROM  transaction_number WHERE PROJET='"+journals.journal1.buss.getText()+"' and NAME='BSC-CDF'";
               }else if(designation.getText().equals("BON D'ENTREE BANQUE")){
               sql="SELECT * FROM  transaction_number WHERE PROJET='"+journals.journal1.buss.getText()+"' and NAME='BEB-USD'";
               }else if(designation.getText().equals("BON DE SORTIE BANQUE")){
               sql="SELECT * FROM  transaction_number WHERE PROJET='"+journals.journal1.buss.getText()+"' and NAME='BSB-USD'";
               } 
             
            pst=con.prepareStatement(sql);
            rs=pst.executeQuery();
           if(rs.next()){
                NUMS=rs.getString("NUM_fiche");
         ...
         try{
              String sqls="SELECT NUM_FICHE FROM recu where TRANSACTION='"+ repalceString+"'and projet='"+journals.journal1.buss.getText()+"'  ORDER BY NUM DESC LIMIT 1";
             //String sql="SELECT NUM FROM etat_de_besoin  WHERE ORIENTATION ='FINANCE' AND BUSS='"+journals.journal1.buss.getText()+"' ORDER BY NUM DESC LIMIT 1";
            ...
            try{
              String sql = null;  
                if(designation.getText().equals("BON D'ENTREE CAISSE")){
                 sql="SELECT NUM_FICHE FROM recu where projet='"+journals.journal1.buss.getText()+"' and TRANSACTION='"+ repalceStrings+"' AND MONAIS='"+journals.journal1.devise.getText()+"'  ORDER BY NUM DESC LIMIT 1";
  ...
  public void Call_for_num_receipt()
    {
       ...
       String  repalceStrings=designation.getText().replace("'","''");
        try{
            String sql="select NUM_FICHE from recu where  PROJET='"+journals.journal1.buss.getText()+"'";
            ...
            if(rs.next()){
             String   MAX_RECEIPT_NO=rs.getString("NUM_FICHE");
                ...
                JOptionPane.showMessageDialog(null, MAX_RECEIPT_NO);    
            }
   ...
   public void showTableDatas()
    ...
    try{
       ...
       String sql="SELECT DESCR as 'Motifs',PT as 'Montant',NAME as 'En faveur de',Num_FICHE,Date FROM recu where TRANSACTION='"+repalceString+"' and NUM='"+num.getText()+"' and projet='"+journals.journal1.buss.getText()+"'";
       pst = con.prepareStatement(sql);
      rs= pst.executeQuery();
         ...
         this.setAlwaysOnTop(false);
            ...
            String NameFile=""+NameFiles+"bon_de_sortie_banque.jrxml";
                ...
                JasperDesign jd=JRXmlLoader.load(NameFile);
                 ...
                 String sql="select*from recu  INNER JOIN projet ON recu.projet=projet.PROJET_NUm  where recu.NUM_FICHE='"+num.getText()+"'";
...
//jp.setFrameIcon(new javax.swing.ImageIcon(<<your image>>));
    ...
    private void initComponents() {
        ...
        designation.setText("BON DE SORTIE BANQUE");
...
Call_for_num_receipt();        // TODO add your handling code here:
    ...
    public static final javax.swing.JTextField busss = new javax.swing.JTextField();
