 ...
 * @author Dosh
 */
public class pv_test extends javax.swing.JPanel {
  Connection con=null;
...
//        
  ...
  {   
   ...
   Pts = Ptss.replace("'", "''");
    try{
            String sqls="select * from pv_test WHERE  NUM='"+numero+"' and QUESTION='"+Docs+"' and offre='"+home_offre.batch.getSelectedValue()+"'";
...
try{
              String sql="select Num,sum(point) as Point,FORMAT(sum(point)*100/"+val+",2) as '_%_' from pv_test  WHERE offre ='"+home_offre.batch.getSelectedValue()+"' group by num";// group by nom having (sum(point)*100)/45 >= '"+jTextField1.getText()+"' order by nom";// ";
  //FORMAT(CREDIT,2)
...
try{
 con=JavaDbConnect.dbConnect(); 
        ...
        try{
                  int roW= doc1.getSelectedRow();
          String numero = (doc1.getModel().getValueAt(roW,0). toString());
            String sql="SELECT  Question,Point FROM pv_test where status='Teste' and offre='"+home_offre.batch.getSelectedValue()+"' and num='"+numero+"'";
...
try{
 con=JavaDbConnect.dbConnect(); 
  pourcentage_test m = new pourcentage_test();
m.show();
