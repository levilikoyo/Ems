...
public void call(){
  ...
  model = (DefaultTableModel) table.getModel();
        ...
        api.listEnginss().enqueue(new Callback<List<Engin1>>(){
        //api.listEngins().enqueue(new Callback<List<Engin1>>(){
            @Override
            public void onResponse(Call<List<Engin1>> call, Response<List<Engin1>> response) {
                for(Engin1 engin : response.body()){
                    model.insertRow(
                            model.getRowCount(), 
       ...
       col2.setPreferredWidth(200);
            ...
            @Override
            public void onFailure(Call<List<Engin1>> call, Throwable t) {
                System.out.println("Erreur 1 : " + t);
            }
        });
 ...
 */
  ...
  /*   String desi=null,descr=null;
      ...
      api.ajouterEngins(desi, descr).enqueue(new Callback<Engin1>(){
            @Override
            public void onResponse(Call<Engin1> call, Response<Engin1> response) {
                Engin1 engin = response.body();
                model.insertRow(
                            model.getRowCount(), 
                            new Object[]{engin.getId(), engin.getDesignation(), engin.getDescription()}
                    );
            ...
            @Override
            public void onFailure(Call<Engin1> call, Throwable t) {
