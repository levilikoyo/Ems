 ...
 * @author Doshe PC
 */
public class add_on_stock_bins extends javax.swing.JFrame {

   Connection con=null;
PreparedStatement pst=null;
ResultSet rs= null;
DefaultTableModel mode; 
    public add_on_stock_bins() {
        initComponents();
...
String    num = (jTable1.getModel().getValueAt(rows,0). toString());
 ...
 public void save(){
      if(jLabel1.getText().equals("Store/ Archive")){
     try{
     ...
     String sql="INSERT INTO Archive_folder (NOM,DATES)"+
             "VALUES(?,?)";
     pst=con.prepareStatement(sql);
      pst.setString(1, textField1.getText());
     ...
     home_archivage1.jPanel6.removeAll();
home_archivage1.jPanel6.repaint();
home_archivage1.jPanel6.revalidate();
    //    setOpaque(false);
        ...
        home_archivage1.jPanel6.setLayout(new MigLayout("inset 0, fillx, wrap", "[fill]"));
 ...
 // DateTimeFormatter dtf = DateTimeFormatter.ofPattern("uuuu/MM/dd HH:mm:ss");
               ...
               home_archivage1.jPanel6.add(new folder(Nom,Dates));
            }
           //  con.close();
        }catch(Exception ex){JOptionPane.showMessageDialog(null, ex);}
...
try{
    ...
    try{
     ...
     }else if(jLabel1.getText().equals("Folder/ Classeur")){
     try{
     String sql="INSERT INTO stock_bin_location (NOM,CAT,DEPOT,RANGES,ETAGERE,LIGNE,BIN,PROFONDEUR)"+
             "VALUES(?,?,?,?,?,?,?,?)";
     pst=con.prepareStatement(sql);
 ...
 public void call(){
     ...
     store_setting.depot.addItem("- Store/dépôt -");
      try{
          String sql="SELECT * FROM stock_bin_location where cat='Store/ Archive' order by Nom";
    ...
    store_setting.profondeur.addItem("- Folder/ Classeur -");
       
    try{
          String sql="SELECT * FROM stock_bin_location where cat='Folder/ Classeur' order by Nom";
             pst = con.prepareStatement(sql);rs=pst.executeQuery();
            while(rs.next()){
                //String sum=rs.getString("nom");
                String sums=rs.getString("Nom");
 ...
 public void call_table(){
           ...
           try{
        PreparedStatement ps = con.prepareStatement("Select * FROM stock_bin_location where cat='"+jLabel1.getText()+"' order by Nom");
...
call();
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(add_on_stock_bins.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new add_on_stock_bins().setVisible(true);
            }
        });
    }
